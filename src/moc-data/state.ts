type MocData = {
    ...
    statusLike?: number;
    favorites?: boolean;
    arr?: Array<MocData> | null | undefined
}
const trialData: Array<MocData> = [
    {
        id: 1,
...
class State {
    ...
    creatClass = (stat: boolean = false): Array<Block> | number => {
        let historyStore: Array<MocData> | null = []
        const local = localStorage.getItem('comments')
        if (local) {
            historyStore = JSON.parse(local)
            this.store = <MocData[]>historyStore
        }
        ...
        if (stat) {
            const id = JSON.parse(<string>localStorage.getItem('comments-id'))
            if (id === null) {
                return 101
            }
            return Number(id)
    ...
    saveData = (obj: Array<Block>, id: number): void => {
